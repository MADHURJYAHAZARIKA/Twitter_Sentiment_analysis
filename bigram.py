...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar  4 22:10:17 2020
...
class VoteClassifier(ClassifierI):
    ...
    def classify(self, vectoriser):
        votes = []
        for c in self._classifiers:
            v = c.classify(vectoriser)
            votes.append(v)
        return mode(votes)

    def confidence(self, vectoriser):
        votes = []
        for c in self._classifiers:
            v = c.classify(vectoriser)
            votes.append(v)
...
training_set = document[5000:]# + document[9662:]
testing_set =  document[:5000]
print(document)
...
classifier = VoteClassifier(NuSVC,LinearSVC,SVC)
